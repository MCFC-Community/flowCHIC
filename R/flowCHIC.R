chic<-function(x,y,show_cluster=TRUE,type="p",main="",col_nmds="black",cex=0.6,pos=1,group,legend_pos="topleft",abiotic,p.max=0.05,col_abiotic="magenta",verbose=FALSE,...){
  newlist<-list()
  x$Label<-gsub("overlap_","",x$Label)  #removes "overlap_" from file labels
  x$Label<-gsub(".bmp","",x$Label)  #removes ".bmp" from file labels
  ...
  dats<-data.frame(Label=x$Label, Label.1=nam[,1], Label.2=nam[,2], IntDen=y$IntDen, Area=x$Area, IntDen.Area=y$IntDen/x$Area/100)  #final data file ("dats") is created by merging and conversion of XOR and overlay data
  ...
  colnames(mat)<-rownames(mat)<-union(unique(dats$Label.1), unique(dats$Label.2))  #label names are written into matrix
  print(colnames(mat))
